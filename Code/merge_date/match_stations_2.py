...
# 第二轮匹配，要求15km内，总文件大小>=40MB的数据量最大的站点
# 匹配356个（新增132个）

# 读取上次生成的匹配文件
airports_df = pd.read_csv('match_stations_1.csv')
...
for idx, airport_row in airports_df[airports_df['Station_ID'].isna()].iterrows():
    ...
    # 使用 KDTree 查找距离机场 15 公里以内的所有站点索引
    nearby_indices = station_tree.query_ball_point(airport_location, max_distance / 111)  # 111 approx km per degree

    # 筛选出距离在 15 公里以内且 Total_Size_MB 大于 40 的站点，并找到数据量最大的站点
    max_total_size = 40  # 设置最小 Total_Size_MB 值为 40
...
# 保存更新后的结果
airports_df.to_csv('match_stations_2.csv', index=False)
